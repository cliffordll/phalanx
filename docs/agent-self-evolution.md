...
phalanx 当前状态（截至 §2.8.b wave 4）：经验流 **95%** 就位（trajectory + sessions + long-term memory + `@reference` 显式引用 + 自动 context compression 全部到位）；评估闭环 **80%**（§2.8.a 落地，10 + 5 = 15 个 golden task，含 reference 类目）；反思 / 更新 仍为零。
...
**§2.8.a + §2.8.b 完成后**：「经验流 → 评估」两段已闭，agent 跨 session 累积经验 + 任何改动有 baseline 可衡量。**剩下最大瓶颈是反思 / critic** —— 需要 §2.8.c delegate / sub-agent 接通后才能跑。
...
3. **delegate / 子 agent**（§2.8.c）— `tools/delegate_tool.py` 解锁 critic / planner 双 agent 模式。前置：memory（✅ 已就绪，critic 能拿到 session 历史）+ guardrails（子 agent 可能并行写盘）。**当前推荐下一站**。设计文档：[`phase-2.8c-delegate.md`](phase-2.8c-delegate.md)。
...
**§2.8.c 准备就绪**——memory + evaluation 都已落地，delegate 落地后立即可以跑"主 agent + critic 互验"模式，并用 `phalanx eval --baseline pre-delegate --diff` 验证 critic 是否真的让平均成功率上升而非纸面热闹。
...
§2.8.a + §2.8.b 已经落地（详见 [`phase-2.8a-evaluation.md`](phase-2.8a-evaluation.md) 和 [`phase-2.8b-memory-context.md`](phase-2.8b-memory-context.md)）。phalanx 现在**第一次具备"跨 session 学习能力 + 改动可衡量"**——闭环图里"经验流→评估"两段半已齐，缺反思与更新。

接下来按 ROI 推荐：

1. **§2.8.c Delegate / 子 agent**（约 5-7 工作日）—— `tools/delegate_tool.py` + `agent/auxiliary_client.py` 异步 surface。前置：memory（✅）+ evaluation（✅）。落地后第一次能跑 critic / reviewer 双 agent，把 reflection 闭环补上。**设计文档已就位**：[`phase-2.8c-delegate.md`](phase-2.8c-delegate.md)。`agent/curator.py` 移到 §2.8.d skills 期一起做（curator 跟 skills 系统耦合）。
2. **§2.8.e Guardrails + checkpoints**（约 3-5 工作日）—— `tools/checkpoint_manager.py` + `agent/tool_guardrails.py`。前置：无。**建议跟 §2.8.c 并行或更早**——sub-agent 一旦能写文件，没 checkpoint 就是裸奔。
3. **§2.8.d Skills 系统**（约 7-10 工作日）—— `skills/` + `tools/skill*.py` + `tools/skills_hub.py` + `propose_skill` 工具。前置：delegate（curator 是独立 agent）。

注意 memory + evaluation 的存在让上面三步**第一次可衡量**——任何 critic / skill / guardrail 改动都能 `phalanx eval --baseline X --diff` 验证有没有让 agent 变笨 / 变慢 / 变贵。这是 §2.8.a 存在的全部理由。
