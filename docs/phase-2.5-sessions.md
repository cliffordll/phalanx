...
| Wave | 内容 | 提交 |
|---|---|---|
| 1 | `hermes_state.py` 骨架 — schema + 连接 + `_execute_write` jitter 重试 + `_init_schema`（含 `_reconcile_columns` / FTS5）+ 9 个核心 CRUD 方法（`create_session` / `end_session` / `reopen_session` / `ensure_session` / `get_session` / `update_system_prompt` / `update_token_counts` / `append_message` / `get_messages`）+ `_encode_content` / `_decode_content` | uncommitted |
