...
python -m hermes_cli oneshot --model gpt-4o-mini --debug "用 echo 工具回显 hi"
...
python -m hermes_cli oneshot --debug "用 <name> 工具完成 X"      # 闭环跑通，debug 输出可读
...
python -m hermes_cli oneshot --provider anthropic --stream "..."   # 强制流式
...
python -m hermes_cli oneshot --resume <id> "..."          # 短链路恢复
```
...
| **1** | 裁剪版 `run_agent.py` + 最小 CLI（`hermes_cli/main.py` + `cli.py` 薄壳）+ `tools/registry.py` + 1 个工具 | `python -m hermes_cli doctor` ✓<br>`python -m hermes_cli tools list` ✓<br>`python -m hermes_cli oneshot --debug "..."` ✓<br>`pytest tests/` 全过 |
...
| 4 | 至少 1 个 adapter + 流式 | `python -m hermes_cli provider test <name>` ✓<br>`python -m hermes_cli oneshot --provider <name> --stream "..."` |
