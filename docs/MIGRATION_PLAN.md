...
- **guardrails + checkpoints** (§2.8.d) — 自主化前的安全网,详细 wave 见 [`phase-2.8d-guardrails.md`](phase-2.8d-guardrails.md)。**wave 1 ✅** `agent/tool_guardrails.py` 骨架(40 单测) / **wave 2 ✅** `tools/checkpoint_manager.py` 三件套快照 + `phalanx checkpoint` CLI + `/snapshot` `/rollback` `/checkpoints` REPL slash(34 单测) / **wave 3 ✅** `SessionDB.event_log` 表(schema_version 12→13)+ `log_event/get_event/query_events/event_count` CRUD + 五类自动 hook(`tool_call_pre/post` 在 `_dispatch_tool_call` / `guardrail_verdict` 在 `_guardrail_check` 的 DENY/APPROVED/USER_DENIED 三分支 / `memory_store` 在 `SessionDB.store_memory` 同事务 / `checkpoint_create` + `rollback` 在 `CheckpointManager`,`config_write`/`skill_create` 留给后续 wave)+ `_audit_event` / `_audit_hash` / `_audit_agent_id`(`<session>#d<depth>`)helper + `phalanx audit log/count/show` CLI(`--since 5m/2h/1d` 或 ISO + `--type` + `--target` LIKE + `--json`)+ REPL `/audit [show] [all|<event_type>]` slash,36 个新单测;wave 4 capability gating + golden 待开
