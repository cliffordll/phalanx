...
- **guardrails + checkpoints** (§2.8.d) — 自主化前的安全网,详细 wave 见 [`phase-2.8d-guardrails.md`](phase-2.8d-guardrails.md)。**wave 1 ✅** `agent/tool_guardrails.py` 骨架:`GuardrailVerdict` 枚举(ALLOW / REQUIRE_APPROVAL / DENY)+ `classify_tool_call` 纯函数(13 条危险命令 regex + 8 条 self-mod 路径前缀 + 9 个 always-allow read-only 工具)+ `ask_for_approval` 三档(yolo / interactive prompt / non-interactive deny)+ `AIAgent.{yolo_mode, enable_self_mod}` flag + `_dispatch_tool_call` 加 `_guardrail_check` 钩子(分类失败默认 ALLOW、approval 失败默认 deny);40 个新单测 / wave 2 checkpoints / wave 3 audit log / wave 4 capability gating + golden 待开
