...
- **guardrails + checkpoints** (§2.8.d) — 自主化前的安全网,详细 wave 见 [`phase-2.8d-guardrails.md`](phase-2.8d-guardrails.md)。**wave 1 ✅** `agent/tool_guardrails.py` 骨架(40 单测) / **wave 2 ✅** `tools/checkpoint_manager.py`:三件套快照(git stash create + SQLite Connection.backup() + config tar.gz)+ `Checkpoint` dataclass + `CheckpointManager.{create,list,get,rollback,delete}` + `~/.phalanx/checkpoints/<ckpt-UTC-rand6>/` layout + `phalanx checkpoint create/list/show/rollback/delete` CLI(--yes 双重确认)+ REPL `/snapshot [name]` `/rollback [id|name]` `/checkpoints` slash + 完整 git 仓库 fixture 测试 stash 路径,34 个新单测;wave 3 audit log + wave 4 capability gating + golden 待开
