...
- **delegate / 子 agent** — 主 agent 派生子 agent 跑独立子任务，共享 IterationBudget（接口已在 §2.1 预留）：`tools/delegate_tool.py`、`agent/auxiliary_client.py`、`agent/curator.py`。详细 wave 见 [`phase-2.8c-delegate.md`](phase-2.8c-delegate.md)。**wave 1 ✅** 骨架 / **wave 2 ✅** 角色化(critic/planner system prompt + `VERDICT:` / `ESTIMATE:` 强制输出 + `extract_verdict()` + `model_override` + `phalanx oneshot --critic-model` CLI + REPL `/critic`,24 个新单测) / **wave 3 ✅** async surface 真实化:`get_async_text_auxiliary_client` / `async_call_llm` / `async_summarize_messages` 不再 stub,接入 `openai.AsyncOpenAI`,与同步 surface 共用 `_resolve_auxiliary_config` 等 helper;新 `_resolve_auxiliary_timeout(task)` 读 `auxiliary.<task>.timeout` config 默认 360s;`tools/web_tools.py` 的 `_call_summarizer_llm` 现在能跑通真实 LLM 摘要而非 fallback truncated raw;23 个新单测含 web_tools 端到端集成
