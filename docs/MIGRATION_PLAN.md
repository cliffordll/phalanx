...
- **delegate / 子 agent** — 主 agent 派生子 agent 跑独立子任务，共享 IterationBudget（接口已在 §2.1 预留）：`tools/delegate_tool.py`、`agent/auxiliary_client.py`、`agent/curator.py`。详细 wave 见 [`phase-2.8c-delegate.md`](phase-2.8c-delegate.md)。**wave 1 ✅**：`tools/delegate_tool.py` 骨架 + `delegate_task` 工具注册 + IterationBudget / `parent_session_id` / `delegation_depth` 三件套继承 + `_DELEGATION_DEPTH_MAX=2` 递归保护 + 失败 wrap 不抛异常 + dispatch 路径加 `caller_agent` kwarg + 27 个新单测
