...
- **delegate / 子 agent** — 主 agent 派生子 agent 跑独立子任务，共享 IterationBudget（接口已在 §2.1 预留）：`tools/delegate_tool.py`、`agent/auxiliary_client.py`、`agent/curator.py`。详细 wave 见 [`phase-2.8c-delegate.md`](phase-2.8c-delegate.md)。**wave 1 ✅** 骨架 / **wave 2 ✅** 角色化:`_ROLE_SYSTEM_PROMPTS` (executor/critic/planner) 通过 `ephemeral_system_prompt` 注入子 agent;critic 强制 `VERDICT: ACCEPT|REJECT|REVISE` 一行结构化输出 + planner 强制 `ESTIMATE: N turn(s)`;`subject_artifact` 在 critic 走系统提示 / executor 走 user message / planner 忽略;`extract_verdict()` 工具方法;`model_override` 让 critic 用更便宜模型;`phalanx oneshot --critic-model X` CLI 主跑后自动 critic 复审;REPL `/critic [help]` 命令复审最近一条 assistant 回复;24 个新单测
