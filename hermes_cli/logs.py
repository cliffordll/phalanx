"""``hermes logs`` — view and filter phalanx log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.hermes/logs/`` (resolved at call time via
...
Ported from upstream ``hermes-agent/hermes_cli/logs.py`` (~390 lines)
verbatim — pure file-reading code with no agent dependencies.  The
``--component`` filter relies on ``hermes_logging.COMPONENT_PREFIXES``
to map a friendly name to the logger-name prefixes that belong to
each component.
"""
...
# Logger name extraction — after level and optional session tag, the
# next non-space token before ":" is the logger name.
...
def tail_log(
    ...
    *,
    ...
    Returns 0 on success, non-zero on user / config errors.
    ...
    if filename is None:
        sys.stderr.write(
            f"Unknown log: {log_name!r}.  Available: "
            f"{', '.join(sorted(LOG_FILES))}\n"
        )
        return 2
    ...
    if not log_path.exists():
        ...
        sys.stderr.write(
            "(Logs are created when phalanx runs — try 'hermes chat' first)\n"
        )
        return 1
    ...
    if component:
        ...
        if component_lower not in COMPONENT_PREFIXES:
            ...
            sys.stderr.write(
                f"Unknown component: {component!r}.  Available: {available}\n"
            )
            return 2
    ...
    try:
        ...
        sys.stderr.write(f"Permission denied: {log_path}\n")
        return 1
    ...
    if follow:
        print(
            f"--- {display_hermes_home()}/logs/{filename}{filter_desc} "
            "(Ctrl+C to stop) ---"
        )
    else:
        print(
...
def _read_tail(
    ...
    *,
    ...
    When filters are active, we read more raw lines than ``num_lines``
    so we still find enough survivors after filtering.
    """
    if has_filters:
...
def _read_last_n_lines(path: Path, n: int) -> list:
    ...
    For files under 1 MB, reads the whole file (fast, simple).  For
    larger files, reads chunks from the end.
    """
    try:
        ...
        with open(path, "rb") as f:
            ...
            while pos > 0 and len(lines) <= n + 1:
                ...
                if lines:
                    # Merge the last partial line of the new chunk with
                    # the first partial line of what we already have.
...
def list_logs() -> int:
    """Print available log files with sizes.  Returns 0."""
