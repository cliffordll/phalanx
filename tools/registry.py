...
class ToolRegistry:
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            if entry.is_async:
                # Phase 1 has no model_tools._run_async yet; fall back to a
                # local asyncio.run, which works for any coroutine-returning
                # handler that doesn't already share an event loop.  When
                # model_tools lands in a later phase, prefer its variant
                # (it knows how to thread-bridge an outer running loop).
                try:
                    from model_tools import _run_async  # type: ignore[import-not-found]
